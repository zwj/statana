#include <improve/performance_time.h>
#include <sys/time.h>
namespace improve 
{
    ...
    {
        double t1, t2;
        t1 =  (double)_impl_ptr->_tstart.tv_sec + (double)_impl_ptr->_tstart.tv_usec/(1000*1000);
        t2 =  (double)_impl_ptr->_tend.tv_sec + (double)_impl_ptr->_tend.tv_usec/(1000*1000);
